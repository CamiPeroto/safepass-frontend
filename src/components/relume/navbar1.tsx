...
export const Navbar1 = (props: Navbar1Props) => {
  ...
  return (
    <section id="relume" className="z-[999] flex w-full items-center border-b border-border-primary bg-background-primary lg:min-h-18 lg:px-[5%]">
        ...
        >
          {navLinks.map((navLink, index) =>
            navLink.subMenuLinks && navLink.subMenuLinks.length > 0 ? (
              ...
              <a key={index} href={navLink.url} className="block py-3 text-md first:pt-7 lg:px-4 lg:py-2 lg:text-base first:lg:pt-2">
                {navLink.title}
              </a>
            )
          )}
...
const SubMenu = ({ navLink, isMobile }: { navLink: NavLink; isMobile: boolean }) => {
  ...
  return (
    <div onMouseEnter={() => !isMobile && setIsDropdownOpen(true)} onMouseLeave={() => !isMobile && setIsDropdownOpen(false)}>
        ...
        <AnimatePresence>
          ...
          >
            {navLink.subMenuLinks?.map((navLink, index) => (
              <a key={index} href={navLink.url} className="block py-3 pl-[5%] text-md lg:px-4 lg:py-2 lg:text-base">
                {navLink.title}
              </a>
            ))}
          </motion.nav>
...
export const Navbar1Defaults: Props = {
  logo: {
    url: "#",
    src: "/images/safepass.png",
    alt: "Logo image",
  },
  navLinks: [
    { title: "Home", url: "/vault/list" },
    // { title: "Link Two", url: "#" },
    // { title: "Link Three", url: "#" },
    {
      title: "Perfil",
      url: "#",
      subMenuLinks: [{ title: "Seu Perfil", url: "#" }],
    },
  ],
  buttons: [
    {
